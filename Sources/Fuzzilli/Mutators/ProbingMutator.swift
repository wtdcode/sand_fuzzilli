    ...
    override func mutate(_ program: Program, using b: ProgramBuilder, for fuzzer: Fuzzer) -> Program? {
        ...
        guard execution.outcome == .succeeded else {
            if case .crashed(let signal) = execution.outcome {
                ...
                fuzzer.processCrash(instrumentedProgram, withArgs: execution.args, withSignal: signal, withStderr: execution.stderr, withStdout: execution.stdout, origin: .local, withExectime: execution.execTime)
