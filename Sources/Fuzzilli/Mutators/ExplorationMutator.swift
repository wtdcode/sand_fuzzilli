...
public class ExplorationMutator: Mutator {
    ...
    override func mutate(_ program: Program, using b: ProgramBuilder, for fuzzer: Fuzzer) -> Program? {
        ...
        case .crashed(let signal):
            ...
            fuzzer.processCrash(instrumentedProgram, withArgs: execution.args, withSignal: signal, withStderr: execution.stderr, withStdout: stdout, origin: .local, withExectime: execution.execTime)
        case .succeeded:
            // The expected case.
            break
        }
